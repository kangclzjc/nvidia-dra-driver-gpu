...
#
# test-dryrun-e2e.sh — Full end-to-end test using real compute-domain-daemon
# in --dry-run mode: CD → daemon startup → CDClique → workload pods.
#
# Measures the complete pipeline from ComputeDomain creation through daemon
# registration, CDClique negotiation, and workload pod scheduling.
...
# Configuration
# ---------------------------------------------------------------------------
TEST_NAME="dryrun-e2e"
CD_NAME="perf-dryrun-e2e"
NUM_RACKS=1
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --pods-per-rack)    PODS_PER_RACK="$2";            shift 2 ;;
        --mode)             MODE="$2";                     shift 2 ;;
        ...
        -h|--help)
            ...
            echo ""
            echo "Full E2E dry-run test: CD → daemon → CDClique → workload pods."
            echo ""
            echo "Options:"
            echo "  --racks N              Number of racks (default: 1)"
            echo "  --nodes-per-rack M     Nodes per rack (default: 18)"
            echo "  --pods-per-rack P      Workload pods per rack (default: 18)"
            ...
            echo "  --cd-name NAME         ComputeDomain name (default: perf-dryrun-e2e)"
...
TOTAL_WORKLOAD_PODS=$(( NUM_RACKS * PODS_PER_RACK ))

log_info "=== Test: ${TEST_NAME} (mode=${MODE}) ==="
log_info "  Racks:              ${NUM_RACKS}"
log_info "  Nodes per rack:     ${SIM_NODES_PER_RACK}"
log_info "  Total daemon nodes: ${SIM_NODES}"
log_info "  Pods per rack:      ${PODS_PER_RACK}"
log_info "  Total workload pods:${TOTAL_WORKLOAD_PODS}"
...
trap 'log_info "Cleaning up ${TEST_NAME}"; cleanup_dryrun_e2e' EXIT

# ---------------------------------------------------------------------------
# Record overall E2E start time
# ---------------------------------------------------------------------------
e2e_start="$(now_ms)"
...
if [[ "${MODE}" == "kind" ]]; then
    ...
    daemon_create_ms="$(elapsed_ms "${daemon_start}")"
    log_info "Node labeling complete: ${daemon_create_ms} ms (${label_count} nodes)"
...
# ---------------------------------------------------------------------------
# 3. Wait for CDClique registration
# ---------------------------------------------------------------------------
log_info "Step 4: Waiting for ${NUM_RACKS} CDCliques to be created"
...
# ---------------------------------------------------------------------------
# 4. Wait for all nodes Ready in CD status
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. Create workload pods (referencing CD ResourceClaimTemplate)
# ---------------------------------------------------------------------------
log_info "Step 7: Creating ${TOTAL_WORKLOAD_PODS} workload pods"

pod_create_start="$(now_ms)"
create_workload_pods "${CD_NAME}" "${TOTAL_WORKLOAD_PODS}" "${BATCH_SIZE}" "${WORKLOAD_IMAGE}"
...
progress_json="$(monitor_pod_progress "cd-name=${CD_NAME}" "${TOTAL_WORKLOAD_PODS}" "${SAMPLE_INTERVAL}" "${TIMEOUT_PODS}")"

# ---------------------------------------------------------------------------
# 9. Wait for all workload pods Running
# ---------------------------------------------------------------------------
all_running_ms="$(wait_for_pods_running "cd-name=${CD_NAME}" "${TOTAL_WORKLOAD_PODS}" "${TIMEOUT_PODS}")"
log_info "All ${TOTAL_WORKLOAD_PODS} workload pods Running: ${all_running_ms} ms"

# ---------------------------------------------------------------------------
# 10. Compute workload pod startup latencies
# ---------------------------------------------------------------------------
...
log_info "Workload pod startup latency: P50=${p50}ms P95=${p95}ms P99=${p99}ms Min=${p_min}ms Max=${p_max}ms (n=${total_pods_measured})"

# Throughput
if (( all_running_ms > 0 )); then
    throughput="$(echo "scale=2; ${TOTAL_WORKLOAD_PODS} * 1000 / ${all_running_ms}" | bc)"
else
    throughput="0"
fi
...
# ---------------------------------------------------------------------------
# 11. Total E2E latency
# ---------------------------------------------------------------------------
total_e2e_ms="$(elapsed_ms "${e2e_start}")"
log_info "Total E2E latency (CD creation → all workload pods Running): ${total_e2e_ms} ms"

# ---------------------------------------------------------------------------
# 12. Collect post-workload metrics
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 13. Per-rack breakdown
# ---------------------------------------------------------------------------
log_info "Step 10: Computing per-rack breakdown"
rack_timings="["
